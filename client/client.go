...
// BlockingClient is using blocking I/O to talk to device
type BlockingClient interface {
	io.Closer
	Read(dest any) error
	// Send sends one message of given CmdId.
	// Payload is serialized to JSON if it's not a string.
	Send(cmd proto.CmdIdType, obj any) error

	// Connect connects to the device and perform handshake if needed
	Connect() error
	IsConnected() bool
	Stats() dto.ProtoStats
}
...
func NewBlockingClient(ver proto.Version, addr string, key []byte, opts ...Opt) BlockingClient {
	c := &clientImpl{
		ver:     ver,
		key:     key,
		origKey: key,
...
func (c *clientImpl) Stats() dto.ProtoStats {
	return c.stats
}
